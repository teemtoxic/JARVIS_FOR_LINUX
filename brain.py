...
#from FUNCTION.Ai_voice import speak
...
def generate(user_query: str, session_cookie: str = "Devs Do Code (Sree)", prints: bool = True) -> str:
    ...
    if response.status_code in (401, 403):
        ...
        cookie_value = list(requests.get("https://pi.ai").cookies)[0].value if requests.get(
            "https://pi.ai").cookies else None
...
def pi_ai_talking(text):
    user_query = text
    print("\033[0;32mJ.A.R.V.I.S: \033[0m", end="", flush=True)
    complete_response = generate(user_query)
    #speak(complete_response, voice="Salli")
